...
const InventoryCards = () => {
  ...
  const handleSearch = (search: string) => {
    ...
    if (!newSearchTerm) {
      setInventoryItems(filteredItems.map(item => ({ ...item, isVisible: true })));
      return;
    }
    ...
    setInventoryItems(inventoryItems.map(item => {
      const isVisible = searchWords.every(word => item.searchableText.includes(word));
  ...
  useEffect(() => {
    ...
    if (partToSearch !== "" && inventoryItems.length > 0) {
...
<section className="mt-5">
      ...
      <div className="sticky top-0 bg-gray-100 bg-opacity-95 py-1">
      ...
      <div id="InventoryCardAdd-div" className="mt-2">
        {showInventoryCardAdd && (
          <InventoryCardAdd
            ...
            onClose={handleCloseInventoryCardAdd} // Pass the onClose handler to InventoryCardAdd
          />
        )}
      </div>
