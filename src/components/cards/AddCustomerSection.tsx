...
interface CustomerCardAddProps {
    ...
    sendCustomerToParent: (newCustomer: customerCards) => void;
  }
  ...
  const { Tax_Registration_Number, Customer_Type, Term, Mobile, Address_Line2, Shipping_Address_Line2, ...restOfCustomer } = newCustomer;

  // Check if all fields, except `Tax_Registration_Number`, are filled
  const allFieldsFilled = Object.values(restOfCustomer).every(value => value.trim() !== "");

  if (!allFieldsFilled) {
    alert("You must fill in all fields but Mobile and Tax Registration Number.");
  ...
  if (newCustomer.Tax_Registration_Number && newCustomer.Tax_Registration_Number.trim() !== "") {
