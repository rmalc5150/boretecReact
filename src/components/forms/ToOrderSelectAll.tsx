...
const ToOrderSelectAll: React.FC = () => {
  ...
  const fetchOrders = async () => {
    ...
    const params: InvokeCommandInput = {
      FunctionName: 'boretec_toOrder_selectAll',
    ...
    <table className="w-full bg-white p-2 rounded-lg text-center border-gray-200 max-h-screen overflow-y-auto">
      <thead>
        <tr>
          ...
          <th className="border-r" onClick={() => sortData('status')}>Payment Method</th>
          <th className="border-r" onClick={() => sortData('status')}>Vendor</th>
          <th className="border-r" onClick={() => sortData('quantity')}>PO Number</th>
          <th className="border-r" onClick={() => sortData('dateCreated')}>Date Created</th>
          <th className="" onClick={() => sortData('origin')}>Origin</th>
        </tr>
      </thead>
      <tbody>
        {parts.map((part, index) => (
          <tr key={index}>
            ...
            <td className="border-r border-t">{part.paymentMethod}</td>
            <td className="border-r border-t">{part.vendorUsed}</td>
            <td className="border-r border-t">{part.qboPoNumber}</td>
            <td className="border-r border-t">{new Date(part.dateCreated).toLocaleDateString()}</td>
            <td className="border-t">{part.origin}</td>
          </tr>
        ))}
      </tbody>
    </table>
